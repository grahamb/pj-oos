...
function findProgramPeriod(program, unit) {
  ...
  periods = getProgramPeriodsThatFitUnitSchedule(unit, program.ProgramPeriods);
  if (!periods.length) {
    return null;
  }
  return chooseProgramPeriod(periods, schedulingAlgorithim);
}
...
const createScheduleForUnit = Promise.coroutine(function* (unit) {
  ...
  while (programSelection.length > 0) {
    ...
    if (program.premium_activity && unitHasPremiumActivity(unit.ProgramPeriods)) {
